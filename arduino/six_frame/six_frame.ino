/*
 * six_frame — Prime_Maxel-v3
 * 6-frame + adjacent primes: {5,6,7,11,12,13} — 93% coprime
 * Coprimality: 93%
 *
 *   Pin 22: f₀/5 = 204.8 Hz (ACTIVE)
 *   Pin 24: f₀/6 = 170.7 Hz (ACTIVE)
 *   Pin 26: f₀/7 = 146.3 Hz (ACTIVE)
 *   Pin 28: f₀/11 = 93.1 Hz (ACTIVE)
 *   Pin 30: f₀/12 = 85.3 Hz (ACTIVE)
 *   Pin 32: f₀/13 = 78.8 Hz (ACTIVE)
 */

#define NUM_CHANNELS 6
const uint8_t outPins[NUM_CHANNELS] = { 22, 24, 26, 28, 30, 32 };
const uint8_t divisors[NUM_CHANNELS] = { 5, 6, 7, 11, 12, 13 };
...
void setup() {
  Serial.begin(9600);
  Serial.println(F("=== six_frame: f₀/5, f₀/6, f₀/7, f₀/11, f₀/12, f₀/13 ==="));
  for (uint8_t i = 0; i < NUM_CHANNELS; i++) {
