/*
 * Progressive Composite Addition #6 — Prime_Maxel-v3
 * EXPERIMENT: 6 composite frequencies active: f₀/4, f₀/6, f₀/8, f₀/9, f₀/10, f₀/12
 *
 *   Pin 22: f₀/4 = 256.0 Hz (ACTIVE)
 *   Pin 24: f₀/6 = 170.7 Hz (ACTIVE)
 *   Pin 26: f₀/8 = 128.0 Hz (ACTIVE)
 *   Pin 28: f₀/9 = 113.8 Hz (ACTIVE)
 *   Pin 30: f₀/10 = 102.4 Hz (ACTIVE)
 *   Pin 32: f₀/12 = 85.3 Hz (ACTIVE)
 */

#define NUM_CHANNELS 6

const uint8_t outPins[NUM_CHANNELS] = { 22, 24, 26, 28, 30, 32 };
const uint8_t divisors[NUM_CHANNELS] = { 4, 6, 8, 9, 10, 12 };
...
void setup() {
  Serial.begin(9600);
  Serial.println(F("=== PROG COMP #6: f₀/4, f₀/6, f₀/8, f₀/9, f₀/10, f₀/12 ==="));
  for (uint8_t i = 0; i < NUM_CHANNELS; i++) {
